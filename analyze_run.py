...
if __name__=="__main__":
    ...
    #Weibull params:
    guess = 0.25 #The guessing rate is 0.25
    flake = 0.99
    slope = 3.5
...
#    for trial_idx,this_trial in enumerate(trial_list):
    for idx_block,i in enumerate(['A','B']):
        ...
        if i == 'A':
           contrast = contrast - p[' annulus_contrast']
        else:
            contrast = contrast[10:]
            this_correct = this_correct[10:]
        ...
        ax.plot(x_for_plot,weibull(x_for_plot,this_fit[0],
                                   ...
                                   flake),
                                   color = 'g')
        ax.set_title('%s task:%s:thresh=%1.2f::slope=%1.2f'
                     %(p['task'],labelit[idx_block],this_fit[0],this_fit[1]))
