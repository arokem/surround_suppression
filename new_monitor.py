...
calib_file = './calibration/NNLGamma_111409_average.txt' # photo data loc
monitor_name = 'NNL' # name of the new monitor
width = None # width of the screen (cm)
distance = None # distance from the screen (cm)
size = [800, 600] # size of the screen (px)
# We can also save notes to our monitor:
notes = """ Here are some notes about this particular monitor setting. """
...
for val in lums.keys():
    ...
    gamma_vals[val] = [calculator.gammaModel[0], 
        calculator.gammaModel[1], calculator.gammaModel[2][0]]
    gammaGrid.append(gamma_vals[val])
# We are not interested in the grayscale value
del gammaGrid[0]

# Create the new monitor, set values and save
newMon = T.Monitor(monitor_name, width, distance)
newMon.setSizePix(size)
newMon.setNotes(notes)
